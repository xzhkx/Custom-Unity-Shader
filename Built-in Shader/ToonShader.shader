Shader "Unlit/ToonShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //Color Clamp to 3 color values.
            //If you want more/less color shading values, just increase/remove an If Statement and adjust the Threshold Values.

            half4 DiffuseThresholdClamp(half4 color)
            {
                 if(color.x <= 0.1) color.x = 0.0;
                 if(color.x <= 0.6 && color.x > 0.1) color.x = 0.4;
                 if(color.x > 0.6) color.x = 0.8;

                 if(color.y <= 0.1) color.y = 0.0;
                 if(color.y <= 0.6 && color.y > 0.1) color.y = 0.4;
                 if(color.y > 0.6) color.y = 0.8;
                    
                 if(color.z <= 0.1) color.z = 0.0 ;
                 if(color.z <= 0.6 && color.z > 0.1) color.z = 0.4;
                 if(color.z > 0.6) color.z = 0.8;

                 return color;
            }

            half4 SpecularThresholdClamp(half4 color)
            {
                 if(color.x <= 0.2) color.x = 0.0;
                 if(color.x <= 0.6 && color.x > 0.2) color.x = 0.4;
                 if(color.x > 0.6) color.x = 0.8;

                 if(color.y <= 0.2) color.y = 0.0;
                 if(color.y <= 0.6 && color.y > 0.2) color.y = 0.4;
                 if(color.y > 0.6) color.y = 0.8;
                    
                 if(color.z <= 0.2) color.z = 0.0 ;
                 if(color.z <= 0.6 && color.z > 0.2) color.z = 0.4;
                 if(color.z > 0.6) color.z = 0.8;
            ...
            fixed4 frag (outputData output) : SV_Target
            {
                ...
                } else 
                {
                    ...
                    diffuse = DiffuseThresholdClamp(diffuse);
                }
                ...
                } else 
                {
                    ...
                    specular = SpecularThresholdClamp(specular);
                }
